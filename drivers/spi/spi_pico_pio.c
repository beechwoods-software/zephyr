			    ...
			    const struct spi_config *spi_cfg)
{
	...
	pio_sm_set_pins_with_mask(data->pio, sm, cpol, BIT(clk->pin));
	pio_sm_set_pins_with_mask(data->pio, sm, 0, BIT(mosi->pin));
...
int spi_pico_pio_init(const struct device *dev)
{
	...
	rc = config_gpio(&dev_cfg->clk_gpio, "clk", GPIO_OUTPUT_INACTIVE);
	if (rc < 0) {
		return rc;
	}

	if (dev_cfg->mosi_gpio.port != NULL) {
		rc = config_gpio(&dev_cfg->mosi_gpio, "mosi", GPIO_OUTPUT_INACTIVE);
		if (rc < 0) {
			return rc;
		}
	}
